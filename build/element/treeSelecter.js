...
KISSY.add('made/mdv/treeSelecter', function (S, DOM, Event, IO, VueRender) {
    return {
        initView: function initView(el, cfg, holder, callback) {
            DOM.addClass(el, 'resourceTree');
            el.innerHTML = '<v-tree data-tree="{{tree}}" data-proxy="{{proxy}}" data-name="{{name}}"            data-isloading="{{isLoading}}" data-isdropdown="{{isDropdown}}"            data-searchable="{{searchable}}" data-folderselectable="{{folderselectable}}"            data-singleselected="{{singleselected}}" data-enableaccordion="{{enableaccordion}}"            data-tracepath="{{tracePath}}" data-threshold="{{threshold}}" data-searchplaceholder="{{searchplaceholder}}"></v-tree>';
