define('mui/mdv-mods/component/pageLoader',function (require, exports, module) {
...
var Mdv = require('mui/mdv/mdv2'), Common = require('../common/common'), MObject = require('../common/object'), MString = require('../common/string'), Dom = require('../common/dom');
var guid = 0;
var Element = function (_Mdv) {
        ...
        function Element(mdvIn) {
            ...
            mdvIn = new Mdv(mdvIn);
            var self = _this;
            self.defineModel('init', Mdv.allOnce([mdvIn.getModel("in") || Mdv.createLiteral(null), mdvIn.getModel("cls"), mdvIn.getModel("cfg")]).thenDemand(function (values) {
            ...
            self.defineModel('load', self.getModel('init').thenDemand(function (initObj) {
                if (initObj.loader) {
                ...
                for (var key in modCfg) {
                    ...
                    case 'loadJsonp':
                        ...
                        cfg.preload = Mdv.createModel(function (resolve) {
                            var h = window, cb = cfg.callback || 'jsonp' + guid++, cb1 = cb;
                            while (cb1.indexOf('.') > 0) {
                        ...
                        }).then(getScript).then(function () {
                            return m;
                        });
                    }
                }
                return Mdv.all([mdvIn.getModel("holder"), mdvIn.getModel("in") || Mdv.createLiteral(null)]).then(function (values) {
                    return function (holder, el) {
                        if (!initObj.cls) {
                            throw 'mdv module not defined';
                        }
...
var Component = function (_Mdv2) {
        _inherits(Component, _Mdv2);
        function Component(mdvIn) {
            _classCallCheck(this, Component);
            var _this2 = _possibleConstructorReturn(this, Object.getPrototypeOf(Component).call(this));
            var self = _this2;
            ...
            self.defineModel(objReg, function (str) {
                return Mdv.createLiteral(str).then(function (str) {
                    return str.replace(/([^'"])(%\{[^\}]+\})/g, function (a, b, c) {
                        return b + '"' + c.replace('"', '\\"') + '"';
                    });
                }).then(MString.eval).then(function (json) {
            ...
            self.defineModel(literalReg, function (name) {
                ...
                return Mdv.createLiteral(literalResult[2]).then(function (str) {
            ...
            self.defineModel(numberReg, function (name) {
                ...
                return Mdv.createLiteral(numberResult[1]).then(MString.eval);
            }, 6);
            //常量布尔值定义模型
            var boolReg = /^\s*(true|false|null)\s*$/;
            self.defineModel(boolReg, function (name) {
                var boolResult = boolReg.exec(name);
                return Mdv.createLiteral(boolResult[1]).then(MString.eval);
            }, 6);
            //存在中间处理过程的模型
            self.defineModel(/->/, function (name) {
                var pos = name.indexOf('->'), process = self.getProcess(name.substring(pos));
            ...
            self.defineModel(propReg, function (name) {
                ...
                if (propResult[1] == '~') {
                    return self.getModel(propResult[2]).then(function (mod) {
                        return Mdv.createModel(function (resolve, reject) {
                            mod.getModel('load').then(function (onResolve, setParams) {
            ...
            self.defineModel(elReg, function (name) {
                ...
                return Mdv.createLiteral(name).then(Dom.getDom).then(function (el) {
            ...
            self.defineModel('datalazyload', function (name) {
                return mdvIn.getModel('mui/datalazyload/').then(function (Datalazyload) {
                    return Datalazyload.instance && Datalazyload.instance() || new Datalazyload(document.body, { autoDestroy: false });
                });
            });
            return _this2;
        }
        _createClass(Component, [{
                key: "getProcess",
                value: function getProcess(name) {
                    //存在中间处理过程的模型
                    var self = this,
                        compResult,
                        compReg = /\s*->\s*(?:\s+([a-zA-Z.0-9_]+(?:\[([0-9]+)\])?\s+))?([#a-zA-Z0-9_\-]+)?\s*(?:\(([a-zA-Z/\-0-9_]+)(?:\:([^\)]+))?\))?(?:\s+([a-zA-Z.0-9_]+(?:\[([0-9]+)\])?)?)?\s*$/;
                    if (compResult = compReg.exec(name)) {
                        var lastName;
        
                        var _ret = function () {
                            var process = function process(model) {
                                var mdvIn = {},
                                    inPorts = compResult[1],
                                    outPorts = compResult[6];
                                if (compResult[5]) {
                                    //按逗号分隔拆分，但忽略引号里面的逗号
                                    var params = compResult[5].split(/,(?=([^']*'[^']*'[^']*)+)$/),
                                        prmResult,
                                        prmReg = /^\s*([a-zA-Z_0-9]+)\s*=\s*(.+)\s*$/;
                                ...
                                return Mdv.createLiteral(compResult[4]).then(function (component) {
                ...
                key: "getComponent",
                value: function getComponent(component, el, cfg) {
                    var self = this,
                        mdvIn = self.mdvIn,
                        comp = /^(.+?)(?:\/\/(.+?)(:(.+))?)?$/.exec(component);
                    return Mdv.createLiteral(comp[1]).then(function (name) {
                        ...
                        return function (mdvIn) {
                            ...
                            if (/m[A-Z]/.test(comp[2])) {
                                ...
                                if (!Cls[comp[2]]) {
                                    throw 'method ' + comp[2] + ' not exists';
                                }
                ...
                key: "addModule",
                value: function addModule(name, loader, cfg) {
                    var self = this;
                ...
                key: "addElement",
                value: function addElement(ele, cls, cfg) {
                    var self = this;
                    Mdv.createLiteral(ele).then(Dom.queryDom).then(function (els) {
                        return Mdv.map(els, function (el) {
                            return Mdv.createLiteral(el).then(function (el, callParams) {
                                ...
                                setTimeout(function () {
                                    self.getModel("datalazyload").then(function (datalazyload) {
                                        datalazyload.addCallback(el, function () {
                                            ...
                                            self.getModel(el).then(function (mod) {
                                                return mod.getModel("load");
                                            }).then();
                                        });
                                    });
